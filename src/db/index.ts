...
const runMigrations = async () => {
  ...
  if (Object.keys(EMBEDDED_MIGRATIONS).length > 0) {
    ...
    try {
      ...
      try {
        const filesColumns = sqlite.query("PRAGMA table_info(files)").all() as Array<{
          name: string;
        }>;
        ...
        if (filesColumns.length > 0 && !hasFileKey && hasTigrisKey) {
          console.log(
            "Files table has old schema (tigris_key), migrating to new schema (file_key)...",
          );
      ...
      const tables = sqlite
        ...
        .all();
      if (tables.length > 0) {
        const count = sqlite.query("SELECT COUNT(*) as count FROM notes").get() as {
          count: number;
        };
        console.log(
          `Database already exists with ${count.count} notes, skipping file-based migrations`,
        );
        return;
      }
