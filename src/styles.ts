...
if (process.env.NODE_ENV === "production") {
  ...
  const possiblePaths = ["./dist/style.css", join(process.cwd(), "dist", "style.css")];

  for (const cssPath of possiblePaths) {
    if (existsSync(cssPath)) {
      EMBEDDED_CSS = readFileSync(cssPath, "utf-8");
      console.log(
        `✅ CSS embedded from ${cssPath} (${(EMBEDDED_CSS.length / 1024).toFixed(2)} KB)`,
      );
      break;
    }
  }
  ...
  if (!EMBEDDED_CSS) {
    console.warn(
      "⚠️  Warning: Could not find style.css to embed. CSS will be served as a separate file.",
    );
  }
}
