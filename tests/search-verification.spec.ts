...
test.describe("Search Verification Test", () => {
  test("creates 10 notes and verifies search returns exactly 1 match", async ({
    page,
    testContext,
  }) => {
    ...
    const testNotes = [
      {
        content: "The magical unicorn galloped through the enchanted forest at dawn.",
        shouldMatch: true,
      },
    ...
    await page
      .waitForFunction(
        (expectedCount) => {
          ...
          const noResultsMsg = document
            .querySelector("#notes-grid p")
            ?.textContent?.includes("No notes found");
    ...
    if (searchResultCount === 0) {
      ...
      for (let i = 0; i < searchResultCount; i++) {
        ...
        console.log(
          `   Note ${i + 1}: "${noteText?.substring(0, 50)}..." ${containsUnicorn ? "(contains unicorn)" : "(NO unicorn!)"}`,
        );
      }
    }
